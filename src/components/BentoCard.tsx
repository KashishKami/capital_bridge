...
const getIcon = (id: string, isHovering: boolean) => {
  ...
  switch (id) {
    case 'technology':
      return <Settings size={size} color={color} className={isHovering ? 'spin-icon' : ''} />;
    case 'quick-quotes':
      return <Zap size={size} color={color} className={isHovering ? 'pulse-icon' : ''} />;
    case 'customer-service':
      return <Headphones size={size} color={color} className={isHovering ? 'bounce-icon' : ''} />;
    default:
...
const BentoCard: React.FC<BentoCardProps> = ({ feature, index }) => {
  ...
  return (
    ...
    >
      ...
      <div>
        <h2 style={{ fontSize: '2rem', marginBottom: '1rem', color: 'var(--text-main)' }}>{feature.title}</h2>
