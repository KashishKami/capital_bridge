...
describe('Content Extraction & Data Layer', () => {
  it('should export a homeContent object that strictly matches the expected TypeScript interface', () => {
    // Hero
    expect(homeContent.hero.headline).toBe('Keep growing your business with Capital Bridge Logistics');
    expect(homeContent.hero.subheadline).toContain('innovative transportation solutions');
    
    // Features
    expect(homeContent.features.length).toBeGreaterThan(0);
    const techFeature = homeContent.features.find(f => f.title === 'Technology');
    ...
    const alisonReview = homeContent.testimonials.find(t => t.author === 'Alison Burgas');
